...
class Trainer:
    def __init__(self, model, train_loader, val_loader, config, device="cuda"):
        ...
        self.scheduler = get_scheduler("linear", self.optimizer, int(config.warmup_ratio * total_steps), total_steps)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                batch = {k: v.to(self.device) for k, v in batch.items()}
                preds = torch.argmax(self.model(**batch)["logits"], dim=1)
